...
export class GetLancarFaltaController implements Controller {
  ...
  async handle(httpRequest: HttpRequest): Promise<HttpResponse> {
    try {
      const { cartao_dia_id, periodoId, statusId, userName } = httpRequest?.body;

      if (!cartao_dia_id) return badRequest(new FuncionarioParamError("Falta id do cartão dia"));
      ...
      this.lancamentoFaltaPostgresRepository.upsert({ cartao_dia_id, periodoId, statusId, userName });
